...
class AdaptivePolicy(object):
    ...
    def minUtilization(self):
        ...
        # [YOUR CODE]
        minimum_utilized_switch = self.utilization.keys()[0]
        for core, value in self.utilization.iteritems():
            print "CORE ---> ", core, value
            if self.utilization[minimum_utilized_switch] > value:
                minimum_utilized_switch = core
        return minimum_utilized_switch
...
class StaticPolicy(object):
    ...
    def build(self, topo):
        ...
        for edge in topo.edgeSwitches.values():
            routingTable[edge.dpid] = []
            for x in topo.hosts.values():
                if x.name in edge.neighbors:
                    outport = topo.ports[edge.name][x.name]
                else:
                    host_vlan = x.vlans[0]
